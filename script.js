...
// Disable simulate until after calculate
...
$('#calcBtn').addEventListener('click', e => {
  ...
  // Plausibility checks
  if (prog==='lifestyle' && meth==='injection')
    return warn('Lifestyle-only cannot use injections.');
  if (prog==='lifestyle' && eff>6)
    warn('Efficacy >6 kg/m² is unlikely for lifestyle-alone programmes.');
  ...
  // ——— Uptake Chart ———
  $('#upText').innerHTML = `<p><strong>${(P*100).toFixed(1)}%</strong> are predicted to choose this programme.</p>`;
  ...
  upChart = new Chart($('#upChart'), {
    ...
    data: {
      ...
      datasets:[{
        ...
        backgroundColor: [getComputedStyle(document.documentElement).getPropertyValue('--uptake-col').trim(), getComputedStyle(document.documentElement).getPropertyValue('--optout-col').trim()]
      }]
    },
    options: {
      responsive: true,
  ...
  cbChart = new Chart($('#cbChart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{ label:'£', data: [tot,saving,qval,net], backgroundColor:['#ff6384','#36a2eb','#ffce56','#4bc0c0'] }]
    },
    options: {
      responsive: true,
...
$('#simBtn').addEventListener('click', e => {
  ...
  const bins=20, min=Math.min(...arr), max=Math.max(...arr), width=(max-min)/bins;
